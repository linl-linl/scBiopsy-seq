...
data <- read.table("DGE_fpkm_clean.txt.gz", header=T, row.names=1)
...
sce <- sc3(sce, ks=2:4, biology=TRUE, gene_filter=T, n_cores=10, 
           d_region_min=0.06, d_region_max=0.25, 
           pct_dropout_min=20, pct_dropout_max=80)
pdf('heatmap.pdf',width=5,height=4)
sc3_plot_expression2(sce, k=2, show_pdata=c("Vol"))
dev.off()

sce2 <- sce[sce@rowRanges@elementMetadata@listData[["sc3_gene_filter"]]=='TRUE', ]
sce2 <- runPCA(sce2)
plotPCA(sce2, colour_by = "Vol")
sce2 <- runTSNE(sce2)
pdf('tSNE.pdf',width=3.8,height=3)
plotTSNE(sce2, colour_by = "Vol")
dev.off()
sce2 <- runUMAP(sce2)
pdf('UMAP.pdf',width=3.8,height=3)
plotUMAP(sce2, colour_by = "Vol")
dev.off()

pp=sce2@int_colData$reducedDims$PCA
rownames(pp)=colnames(data3)
write.table(pp, 'PCA.txt', sep='\t')
